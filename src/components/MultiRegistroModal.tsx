...
export const MultiRegistroModal: React.FC<MultiRegistroModalProps> = ({
  ...
  const existingDailyBySub = useMemo(() => {
    const map: Record<string, { id: string; amount: number }> = {};
    if (!ventasCategory || !dailyDate) return map;
    for (const mov of movements) {
      if (mov.category_id !== ventasCategory.id) continue;
      if (mov.type !== "ingreso") continue;
    ...
    return map;
  }, [movements, ventasCategory, dailyDate]);

  const handleDailyDateBlur = () => {
    if (!dailyDate) return;
    setDailyAmounts(() =>
      Object.keys(existingDailyBySub).reduce<Record<string, string>>((acc, s) => {
        acc[s] = existingDailyBySub[s].amount.toLocaleString("es-ES", {
          minimumFractionDigits: 2,
        });
        return acc;
      }, {}),
  ...
  const selectedSub = ventasCategory?.subcategories.find(
    ...
    const entries = Object.entries(amounts)
      ...
      .map(([dayStr, v]) => {
        ...
        return {
          ...
          category: ventasCategory?.name ?? "Ventas",
  ...
  const handleSaveDaily = () => {
    ...
    const entries = Object.entries(dailyAmounts)
      ...
      .map(([subId, v]) => {
        const amount = parseAmount(v);
        const sub = ventasCategory.subcategories.find(
          (s) => String(s.id) === subId,
        );
        const existing = existingDailyBySub[subId];
        return {
          id: existing?.id,
          date: dailyDate,
          category_id: ventasCategory.id,
          category: ventasCategory.name,
  ...
  return (
    <div className="fixed inset-0 z-60 flex items-center justify-center p-4 bg-on-background/40 backdrop-blur-sm animate-fade-in">
      <div className="bg-surface-container-lowest border-2 border-outline-variant rounded-xl w-full max-w-lg overflow-hidden shadow-2xl transform transition-all max-h-[90vh] flex flex-col">
        {/* Header */}
        <div className="p-4 md:p-typography flex items-center justify-between text-white bg-secondary">
          <div className="flex items-center gap-3">
            <span className="material-symbols-outlined text-[28px]">add_circle</span>
            <h4 className="font-bold text-xl md:text-2xl">Añadir Ingreso</h4>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-4 md:p-6 space-y-4 overflow-y-auto">
          {step === "mode" && (
            <div className="space-y-3">
              ...
              >
                ...
                Registro múltiple (un apunte por día del mes)
              </button>
              <button
                type="button"
              ...
              {!ventasCategory ? (
                ...
                <>
                  <p className="font-medium text-on-surface">
                    ...
                    <span className="font-bold">{ventasCategory.name}</span>
                  </p>
                  ...
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      className="w-full h-14 px-4 bg-surface border-2 border-outline-variant rounded-xl focus:border-secondary focus:outline-none font-medium text-base cursor-pointer"
                    />
                  </div>
                  ...
                  <div className="space-y-1">
                    ...
                    >
                      ...
                      {ventasCategory.subcategories.map((s) => (
          ...
          {step === "daily" && ventasCategory && (
            <div className="space-y-4">
              ...
              <div className="border-2 border-outline-variant rounded-xl divide-y divide-outline-variant overflow-hidden max-h-64 overflow-y-auto">
                {ventasCategory.subcategories.map((sub) => (
